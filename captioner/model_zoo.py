...
def getPositionalEncodingWeights(input_dim, output_dim, name='', verbose=True):
    ...
    position_enc = np.array(
        [[pos / np.power(10000, 2. * i / output_dim) for i in range(output_dim)] for
         pos in range(input_dim)])
...
class Captioning_Model(Model_Wrapper):
    ...
    def __init__(self, params, model_type='Captioning_Model', verbose=1,
        ...
        if params['TRG_PRETRAINED_VECTORS'] is not None:
            if self.verbose > 0:
                logging.info("<<< Loading pretrained word vectors from: " + params[
                    'TRG_PRETRAINED_VECTORS'] + " >>>")
            trg_word_vectors = np.load(
                os.path.join(params['TRG_PRETRAINED_VECTORS'])).item()
            self.trg_embedding_weights = np.random.rand(
                params['OUTPUT_VOCABULARY_SIZE'],
                params['TARGET_TEXT_EMBEDDING_SIZE'])
            for word, index in iteritems(
                    self.vocabularies[self.ids_outputs[0]]['words2idx']):
    ...
    def setOptimizer(self, **kwargs):
        ...
        if int(self.params.get('ACCUMULATE_GRADIENTS', 1)) > 1 and self.params['OPTIMIZER'].lower() != 'adam':
            logging.warning(
                'Gradient accumulate is only implemented for the Adam optimizer. Setting "ACCUMULATE_GRADIENTS" to 1.')
            self.params['ACCUMULATE_GRADIENTS'] = 1

        optimizer_str = '\t LR: ' + str(self.params.get('LR', 0.01)) + \
                        '\n\t LOSS: ' + str(
            self.params.get('LOSS', 'categorical_crossentropy'))

        if self.params.get('USE_TF_OPTIMIZER',
                           False) and K.backend() == 'tensorflow':
            if self.params['OPTIMIZER'].lower() not in ['sgd', 'adagrad', 'adadelta',
                                                        'rmsprop', 'adam']:
                logging.warning(
                    'The optimizer %s is not natively implemented in Tensorflow. Using the Keras version.' % (str(self.params['OPTIMIZER'])))
        ...
        if self.params.get('USE_TF_OPTIMIZER', False) and K.backend() == 'tensorflow' and self.params['OPTIMIZER'].lower() in ['sgd', 'adagrad', 'adadelta', 'rmsprop', 'adam']:
            ...
            if self.params['OPTIMIZER'].lower() == 'sgd':
                if self.params.get('MOMENTUM') is None:
                    optimizer = TFOptimizer(tf.train.GradientDescentOptimizer(
                        self.params.get('LR', 0.01)))
                else:
                    optimizer = TFOptimizer(
                        tf.train.MomentumOptimizer(self.params.get('LR', 0.01),
                                                   self.params.get('MOMENTUM', 0.0),
                                                   use_nesterov=self.params.get(
                ...
                optimizer = TFOptimizer(
                    tf.train.AdamOptimizer(learning_rate=self.params.get('LR', 0.01),
                ...
                optimizer = TFOptimizer(
                    tf.train.AdagradOptimizer(self.params.get('LR', 0.01)))

            elif self.params['OPTIMIZER'].lower() == 'rmsprop':
                optimizer = TFOptimizer(
                    tf.train.RMSPropOptimizer(self.params.get('LR', 0.01),
            ...
            if self.params['OPTIMIZER'].lower() == 'sgd':
                ...
                optimizer = SGDHD(lr=self.params.get('LR', 0.002),
                                  ...
                                  clipvalue=self.params.get('CLIP_V', 0.),
                                  hypergrad_lr=self.params.get('HYPERGRAD_LR',
                                                               0.001))
        ...
        if self.verbose > 0:
            logging.info(
                "Preparing optimizer and compiling. Optimizer configuration: \n" + optimizer_str)
    ...
    def __str__(self):
        ...
        obj_str += '\n'
        obj_str += 'Params:\n\t'
        obj_str += "\n\t".join([str(key) + ": " + str(self.params[key]) for key in
    ...
    def AttentionRNNEncoderDecoder(self, params):
        ...
        if params['RNN_ENCODER_HIDDEN_SIZE'] > 0:
            if params['BIDIRECTIONAL_ENCODER']:
                encoder = Bidirectional(
                    eval(self.use_CuDNN + params['ENCODER_RNN_TYPE'])(params['RNN_ENCODER_HIDDEN_SIZE'],
            ...
            for n_layer in range(1, params['N_LAYERS_ENCODER']):
                if params['BIDIRECTIONAL_DEEP_ENCODER']:
                    current_input_video = Bidirectional(eval(self.use_CuDNN + params['ENCODER_RNN_TYPE'])(params['RNN_ENCODER_HIDDEN_SIZE'],
                                                                                                          kernel_regularizer=l2(params['RECURRENT_WEIGHT_DECAY']),
                                                                                                          recurrent_regularizer=l2(params['RECURRENT_WEIGHT_DECAY']),
                                                                                                          bias_regularizer=l2(params['RECURRENT_WEIGHT_DECAY']),
                                                                                                          kernel_initializer=params['INIT_FUNCTION'],
        ...
        if len(params['INIT_LAYERS']) > 0:
            ...
            initial_state = Dense(params['RNN_DECODER_HIDDEN_SIZE'],
                                  name='initial_state',
                                  kernel_initializer=params['INIT_FUNCTION'],
                                  kernel_regularizer=l2(params['WEIGHT_DECAY']),
                                  bias_regularizer=l2(params['WEIGHT_DECAY']),
        ...
        sharedAttRNNCond = eval('Att' + params['DECODER_RNN_TYPE'] + 'Cond')(params['RNN_DECODER_HIDDEN_SIZE'],
        ...
        if params['DOUBLE_STOCHASTIC_ATTENTION_REG'] > 0:
            alpha_regularizer = AlphaRegularizer(
                alpha_factor=params['DOUBLE_STOCHASTIC_ATTENTION_REG'])(alphas)

        [proj_h, shared_reg_proj_h] = Regularize(proj_h, params, shared_layers=True,
                                                 name='proj_h0')
        ...
        for n_layer in range(1, params['N_LAYERS_DECODER']):
            ...
            shared_proj_h_list.append(
                eval(params['DECODER_RNN_TYPE'].replace('Conditional', '') + 'Cond')(params['RNN_DECODER_HIDDEN_SIZE'],
                                                                                     kernel_regularizer=l2(params['RECURRENT_WEIGHT_DECAY']),
                                                                                     recurrent_regularizer=l2(params['RECURRENT_WEIGHT_DECAY']),
                                                                                     conditional_regularizer=l2(params['RECURRENT_WEIGHT_DECAY']),
                                                                                     bias_regularizer=l2(params['RECURRENT_WEIGHT_DECAY']),
            ...
            [current_proj_h, shared_reg_proj_h] = Regularize(current_proj_h, params,
                                                             shared_layers=True, name='proj_h' + str(n_layer))
        ...
        for i, (activation, dimension) in enumerate(params['DEEP_OUTPUT_LAYERS']):
            shared_deep_list.append(
                TimeDistributed(Dense(dimension, activation=activation,
                                      kernel_initializer=params['INIT_FUNCTION'],
                                      kernel_regularizer=l2(params['WEIGHT_DECAY']),
                                      bias_regularizer=l2(params['WEIGHT_DECAY']),
                                      trainable=params.get('TRAINABLE_DECODER', True),
        ...
        shared_FC_soft = TimeDistributed(Dense(params['OUTPUT_VOCABULARY_SIZE'],
                                               ...
                                               kernel_regularizer=l2(params['WEIGHT_DECAY']),
                                               bias_regularizer=l2(params['WEIGHT_DECAY']),
                                               trainable=(params.get('TRAINABLE_DECODER',
                                                          True) or params.get('TRAIN_ONLY_LAST_LAYER', True)),
                                               ),
        ...
        if params['BEAM_SEARCH']:
            ...
            for (rnn_decoder_layer, proj_h_reg) in zip(shared_proj_h_list,
                                                       shared_reg_proj_h_list):
            ...
            for (reg_out_layer_mlp, reg_out_layer_ctx, reg_out_layer_emb) in zip(
    ...
    def Transformer(self, params):
        ...
        for n_block in range(params['N_LAYERS_ENCODER']):
            src_multihead = MultiHeadAttention(params['N_HEADS'],
                                               ...
                                               name='src_MultiHeadAttention_' + str(n_block))([src_residual_multihead, src_residual_multihead])
            # Regularize
            src_multihead = Dropout(params['DROPOUT_P'])(src_multihead)
            # Add
            if add_first or n_block > 0:
            ...
            # And norm
            src_multihead = BatchNormalization(mode=1,
                                               name='src_Normalization_FF_' + str(n_block))(src_multihead)
        ...
        max_len = params['MAX_OUTPUT_TEXT_LEN']
        ...
        for n_block in range(params['N_LAYERS_DECODER']):
            ...
            shared_trg_multihead = MultiHeadAttention(params['N_HEADS'],
                                                      ...
                                                      dropout=params.get('ATTENTION_DROPOUT_P', 0.),
                                                      mask_future=True,
                                                      # Avoid attending on future sequences
            ...
            # Add
            shared_trg_multihead_add = Add(
                name='trg_Residual_MultiHeadAttention_' + str(n_block))
            shared_trg_add_multihead_list.append(shared_trg_multihead_add)

            # And norm
            shared_trg_multihead_norm = BatchNormalization(mode=1,
                                                           name='trg_Normalization_MultiHeadAttention_' + str(n_block))
            ...
            # Add
            shared_src_trg_multihead_add = Add(
                name='src_trg_Residual_MultiHeadAttention_' + str(n_block))
            shared_src_trg_add_multihead_list.append(shared_src_trg_multihead_add)

            # And norm
            shared_src_trg_multihead_norm = BatchNormalization(mode=1,
                                                               name='src_trg_Normalization_MultiHeadAttention_' + str(n_block))
            shared_src_trg_norm_multihead_list.append(shared_src_trg_multihead_norm)

            # FF
            shared_ff_src_trg_multihead = TimeDistributed(
                PositionwiseFeedForwardDense(params['FF_SIZE'],
            ...
            # Add
            shared_ff_src_trg_multihead_add = Add(
                name='src_trg_Residual_FF_' + str(n_block))
            shared_add_ff_list.append(shared_ff_src_trg_multihead_add)

            # And norm
            shared_ff_src_trg_multihead_norm = BatchNormalization(mode=1,
                                                                  name='src_trg_Normalization_FF_' + str(n_block))
        ...
        shared_FC_soft = TimeDistributed(Dense(params['OUTPUT_VOCABULARY_SIZE'],
                                               ...
                                               kernel_regularizer=l2(params['WEIGHT_DECAY']),
                                               bias_regularizer=l2(params['WEIGHT_DECAY']),
                                               trainable=(params.get('TRAINABLE_DECODER', True) or params.get(
