...
counts_lists = ['train_feat_counts.txt', 'val_feat_counts.txt',
                'test_feat_counts.txt']
...
for f, fc, o, c in zip(features_files, features_counts, out_lists, counts_lists):
    ...
    for enum, line in enumerate(f):
        ...
        this_path = "%s/video_%0.4d" % (
            path_features + '/' + features_name, c_videos)
        if not os.path.isdir(base_path + this_path):
            os.makedirs(base_path + this_path)
        this_path = "%s/video_%0.4d/frame_%0.4d.npy" % (
            path_features + '/' + features_name, c_videos, c_frame)
        # Save array in disk
        try:
