...
def parse_args():
    parser = argparse.ArgumentParser("Interactive video captioning server.")
    parser.add_argument("-ds", "--dataset", required=True, help="Dataset instance")
    parser.add_argument("-s", "--split", required=False, default='test', help="Split for loading features.")
    ...
    parser.add_argument("-ch", "--changes", nargs="*", help="Changes to the config. Following the syntax Key=Value",
                        default="")
    parser.add_argument("-o", "--online", action='store_true', default=False, required=False,
...
class NMTHandler(BaseHTTPServer.BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        for aa in args:
            cc = aa.split('=')
            if cc[0] == 'vid_idx':
                video_idx = urllib.unquote_plus(cc[1])
        ...
        if learn and validated_prefix is not None and video_idx is not None:
            self.server.sampler.learn_from_sample(video_idx, validated_prefix)
            self.send_response(200)  # 200: ('OK', 'Request fulfilled, document follows')
        else:
            hypothesis = self.server.sampler.generate_sample(video_idx, validated_prefix=validated_prefix)
...
class VideoDescSampler:
    def __init__(self, models, dataset, params, params_prediction, params_training, model_tokenize_f, model_detokenize_f, general_tokenize_f,
                 general_detokenize_f, split='test', mapping=None, word2index_x=None, word2index_y=None, index2word_y=None,
        ...
        # Compile sampling function by generating a fake sample # TODO: Find a better way of doing this
    ...
    def generate_sample(self, n_sample, validated_prefix=None, max_N=5, isle_indices=None,
        ...
        current_input = self.dataset.getX_FromIndices(self.split, [n_sample],
                                                      normalization_type=self.params_prediction.get('normalization_type'),
                                                      normalization=self.params_prediction.get('normalize', False),
                                                      dataAugmentation=self.params_prediction.get('data_augmentation', False),
                                                      wo_da_patch_type=self.params_prediction.get('wo_da_patch_type', 'whole'),
                                                      da_patch_type=self.params_prediction.get('da_patch_type', 'resize_and_rndcrop'),
                                                      da_enhance_list=self.params_prediction.get('da_enhance_list', None))[0][0]
        ...
        trans_indices, costs, alphas = self.interactive_beam_searcher.sample_beam_search_interactive(current_input,
    ...
    def learn_from_sample(self, n_sample, target_sentence):
        ...
        if self.online_trainer is not None:
            self.online_trainer.train_online([np.asarray([current_input]), state_below], trg_seq, trg_words=[target_sentence])
        else:
...
def main():
    ...
    parameters_prediction = {'max_batch_size': parameters['BATCH_SIZE'],
                         'n_parallel_loaders': parameters['PARALLEL_LOADERS'],
                         'predict_on_sets': [args.split],
                         'beam_size': parameters['BEAM_SIZE'],
                         'maxlen': parameters['MAX_OUTPUT_TEXT_LEN_TEST'],
                         'optimized_search': parameters['OPTIMIZED_SEARCH'],
                         'model_inputs': parameters['INPUTS_IDS_MODEL'],
                         'model_outputs': parameters['OUTPUTS_IDS_MODEL'],
                         'dataset_inputs': parameters['INPUTS_IDS_DATASET'],
                         'dataset_outputs': parameters['OUTPUTS_IDS_DATASET'],
                         ...
                         'da_patch_type': parameters.get('DA_PATCH_TYPE', 'resize_and_rndcrop'),
                         'da_enhance_list': parameters.get('DA_ENHANCE_LIST', None),
                         'pos_unk': parameters.get('POS_UNK', None),
                         'heuristic': parameters.get('HEURISTIC', None),
                         'search_pruning': parameters.get('SEARCH_PRUNING', False),
                         'state_below_index': -1,
                         'output_text_index': 0,
                         'apply_tokenization': parameters.get('APPLY_TOKENIZATION', False),
                         'tokenize_f': eval('dataset.' + parameters.get('TOKENIZATION_METHOD', 'tokenize_none')),
                         'apply_detokenization': parameters.get('APPLY_DETOKENIZATION', True),
                         'detokenize_f': eval('dataset.' + parameters.get('DETOKENIZATION_METHOD', 'detokenize_none')),
                         'coverage_penalty': parameters.get('COVERAGE_PENALTY', False),
    ...
    excluded_words = None
    interactive_beam_searcher = VideoDescSampler(models, dataset, parameters, parameters_prediction, parameters_training,
